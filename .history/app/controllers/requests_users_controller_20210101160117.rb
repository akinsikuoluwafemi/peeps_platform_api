class RequestsUsersController < ApplicationController
  ...
  def re_publish
    ...
    @republish = RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) > 3 & count(request_id < 5)').where(updated_at: '2020-12-31T21:51:22.311Z')
