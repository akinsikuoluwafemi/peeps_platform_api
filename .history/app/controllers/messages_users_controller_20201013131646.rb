class MessagesUsersController < ApplicationController
  ...
  def index
    ...
    #messages_users = Message.group(:message_id)
...
def create
    ...
    if messages_user.between(messages_user_params).present?
        @messages_user = messages_user.between(messages_user_params).first
    else
      @messages_user = MessagesUser.create!(messages_user_params)
    end
end
  ...
  private
    ...
    def messages_user_params
      ...
      params.require(:messages_user).permit(:sender_id, :recipient_id)
