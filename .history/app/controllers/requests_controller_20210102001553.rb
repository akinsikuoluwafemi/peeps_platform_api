class RequestsController < ApplicationController
  ...
  def index
    ...
    @requests =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("updated_at < ?", @now)
    render json: @requests
  end


  def re_publish
    #  @republish = RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) == 5') && Request.where(active: true)
    @republish = RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) > 3 & count(request_id < 5)') #& RequestsUser.where(updated_at: current_user.updated_at < (Time.now - 24.hours))
    @requests =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("updated_at < ?", @now)
