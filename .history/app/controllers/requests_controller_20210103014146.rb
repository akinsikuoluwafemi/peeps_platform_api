class RequestsController < ApplicationController
  ...
  def index
    ...
    @requests =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("upda_at > ?", @now).where(fulfilled: false)
  ...
  def re_publish
    @now = Time.now - 24.hours
    @republish =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("upda_at < ?", @now).where(fulfilled: false)
