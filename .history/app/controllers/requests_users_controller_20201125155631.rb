class RequestsUsersController < ApplicationController
  ...
  def samevolunteer
    ...
    # @click = RequestsUser.where(user_id:current_user, request_id:current_req).count == 2
    @click = RequestsUser.where(user_id: current_user, request_id: current_user.request.id).count > 1
    if @click 
      render json: true
  ...
  # def samevolunteer
  #   @click = RequestsUser.group(:user_id, :request_id).having("count(user_id) > 0")
