class RequestsUsersController < ApplicationController
  ...
  def republish
    #  @republish = RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) == 5') && Request.where(active:false)
     @republish = RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) > 3 & count(request_id < 5)') && RequestsUser.where(updated_at: current_user.updated_at )
