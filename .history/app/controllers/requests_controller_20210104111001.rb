...
# render json: @inactiveRequests  
  ...
  def success_and_republish_request
     @now = Time.now - 24.hours
    @successAndRepublish =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("updated_at < ?", @now).where(fulfilled: false, user_id).or(Request.where(fulfilled: true))
