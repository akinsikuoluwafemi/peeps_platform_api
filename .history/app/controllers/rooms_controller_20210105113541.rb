class RoomsController < ApplicationController
  ...
  def index
    ...
    @rooms =  RequestsUser.left_outer_joins(:request).group(:request_id).having('count(request_id) < 5') && Request.where("updated_at > ?", @now).where(fulfilled: false)

    # @rooms = Room.group(:request_id).having('count(request_id) < 5').where("updated_at > ?", @now)
    render json: @rooms
